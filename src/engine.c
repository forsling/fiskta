...
typedef struct { int name_idx; i64 pos; } LabelWrite;
...
static enum Err resolve_loc_expr(const LocExpr *loc, const Program *prg, File *io, 
  ...
  switch (loc->base) {
    ...
    case LOC_NAME: {
      ...
      bool found = false;
      for (int i = 0; i < 32; i++) {
        if (vm->labels[i].in_use && 
            strcmp(vm->labels[i].name, prg->names[loc->name_idx]) == 0) {
  ...
  for (int i = 0; i < label_count; i++) {
    ...
    for (int j = 0; j < 32; j++) {
      if (vm->labels[j].in_use && 
          strcmp(vm->labels[j].name, prg->names[name_idx]) == 0) {
        slot = j;
        break;
      }
    }
