...
static enum Err parse_op(char **tokens, int *idx, int token_count, Op *op, Program *prg) {
  ...
  if (strcmp(cmd, "find") == 0) {
    ...
    if (strlen(needle) == 0) return E_BAD_NEEDLE;
    
    op->u.find.needle = malloc(strlen(needle) + 1);
    if (!op->u.find.needle) return E_OOM;
    ...
    if (strcmp(arg, "to") == 0) {
      ...
      if (!op->u.take_until.needle) return E_OOM;
    ...
    if (!is_valid_label_name(name)) return E_LABEL_FMT;
...
static enum Err parse_signed_number(const char *token, int *sign, u64 *n, enum Unit *unit) {
  if (!token || strlen(token) == 0) return E_PARSE;
  ...
  // Parse number
  if (!isdigit(*p)) return E_PARSE;
  
  u64 num = 0;
  while (isdigit(*p)) {
    u64 new_num = num * 10 + (*p - '0');
    if (new_num < num) return E_PARSE; // Overflow
    num = new_num;
    p++;
  }
  ...
  if (*p != '\0') return E_PARSE; // Extra characters
